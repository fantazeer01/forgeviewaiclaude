...
def make_event(asset_prefix, boundary, outcomes=("Up", "Down"), prices=("0.55", "0.45"),
    ...
    return {
        ...
        "title": f"{asset_prefix.upper()} Up or Down",
        "endDate": end_date,
        "markets": [
            {
...
def test_fetch_events_by_slug_sends_slug_params_and_handles_list_response(mocker):
    fetcher = MarketFetcher()
    resp = mocker.Mock()
    resp.raise_for_status = mocker.Mock()
    resp.json.return_value = [{"slug": "btc-updown-5m-1"}]
    get_mock = mocker.patch.object(fetcher.session, "get", return_value=resp)
    result = fetcher._fetch_events_by_slug(["btc-updown-5m-1", "eth-updown-5m-1"])
    assert result == [{"slug": "btc-updown-5m-1"}]
    called_params = get_mock.call_args.kwargs["params"]
    assert called_params == [("slug", "btc-updown-5m-1"), ("slug", "eth-updown-5m-1")]


def test_fetch_events_by_slug_handles_dict_response(mocker):
    fetcher = MarketFetcher()
    resp = mocker.Mock()
    resp.raise_for_status = mocker.Mock()
    resp.json.return_value = {"events": [{"slug": "btc-updown-5m-1"}]}
    mocker.patch.object(fetcher.session, "get", return_value=resp)
    assert fetcher._fetch_events_by_slug(["btc-updown-5m-1"]) == [{"slug": "btc-updown-5m-1"}]


def test_parse_event_maps_up_down_labels_to_yes_no_prices():
    fetcher = MarketFetcher()
    event = make_event("btc", 1782945000, outcomes=("Up", "Down"), prices=("0.62", "0.38"))
    parsed = fetcher._parse_event(event, "BTC", time_module_now())
...
def test_parse_event_returns_none_when_no_markets():
    fetcher = MarketFetcher()
    event = {"slug": "btc-updown-5m-1", "markets": []}
    assert fetcher._parse_event(event, "BTC", time_module_now()) is None


def test_parse_event_returns_none_when_market_closed():
    fetcher = MarketFetcher()
    event = make_event("btc", 1782945000, closed=True)
    assert fetcher._parse_event(event, "BTC", time_module_now()) is None


def test_parse_event_defaults_prices_when_labels_unrecognized():
    fetcher = MarketFetcher()
    event = make_event("btc", 1782945000, outcomes=("Yes", "No"), prices=("0.7", "0.3"))
    parsed = fetcher._parse_event(event, "BTC", time_module_now())
    assert parsed["yes_price"] == 0.5
    assert parsed["no_price"] == 0.5


def test_parse_event_falls_back_to_default_minutes_on_bad_date():
    fetcher = MarketFetcher()
    event = make_event("btc", 1782945000)
    event["markets"][0]["endDate"] = "not-a-date"
    parsed = fetcher._parse_event(event, "BTC", time_module_now())
...
def test_get_active_5min_markets_filters_and_parses(mocker):
    ...
    events = [
        make_event("btc", boundary, prices=("0.55", "0.45")),
        make_event("eth", boundary, prices=("0.40", "0.60")),
        {"slug": "xrp-updown-5m-" + str(boundary), "markets": []},
    ]
    mocker.patch.object(fetcher, "_fetch_events_by_slug", return_value=events)
...
def test_get_active_5min_markets_returns_empty_on_error(mocker):
    fetcher = MarketFetcher()
    mocker.patch.object(fetcher, "_fetch_events_by_slug", side_effect=RuntimeError("boom"))
