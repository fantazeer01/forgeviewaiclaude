"""Online-learning momentum signal: River LogisticRegression over the two
momentum features, trained on every resolved outcome, never reset. Weights
persist to disk so learning survives process restarts."""
...
from config.settings import MOMENTUM_WEIGHTS_FILE

logger = logging.getLogger(__name__)

FEATURE_KEYS = ["price_momentum_1m", "price_momentum_5m"]


class MomentumModel:
    def __init__(self, weights_file: str = MOMENTUM_WEIGHTS_FILE):
    ...
    def _load(self):
        if os.path.exists(self.weights_file):
            try:
                ...
                logger.warning(f"MomentumModel load error, starting fresh: {e}")
    ...
    def save(self):
        try:
            ...
            logger.error(f"MomentumModel save error: {e}")
