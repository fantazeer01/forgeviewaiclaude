...
def fire_repricing_signal(detector, market_id="m1"):
    """Seeds detector price history so the next process_market() call fires
    a real repricing signal (same pattern as test_repricing_detector.py)."""
    old_ts = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(seconds=70)
    detector._price_history[market_id] = [{"ts": old_ts, "yes": 0.9, "no": 0.1}]
...
def test_process_market_updates_feature_history_even_without_signal(tmp_path):
    ...
    fetcher = FakeFetcher()
    qsg = QuantSignalGenerator(detector, state, fetcher, dataset=FakeDataset(), model=None)
...
def test_process_market_fires_signal_identically_to_repricing_logic(tmp_path):
    ...
    fire_repricing_signal(detector)
...
def test_process_market_does_not_use_model_to_decide_signal(tmp_path):
    # even a very low-confidence model prediction must not suppress a real
    # repricing signal, and a very high one must not manufacture a signal
    # that the repricing rule itself did not produce
    detector = RepricingDetector()
    state = StateManager(state_file=str(tmp_path / "state.json"))
    fetcher = FakeFetcher(order_book_top={"best_bid_price": 0.49, "best_ask_price": 0.51,
                                           "total_bid_depth": 100.0, "total_ask_depth": 50.0})
    low_confidence_model = FakeModel(probability=0.01)
    qsg = QuantSignalGenerator(detector, state, fetcher, dataset=FakeDataset(), model=low_confidence_model)
    fire_repricing_signal(detector)
    signal = qsg.process_market(make_market(yes_price=0.5, no_price=0.5))
    assert signal is not None  # repricing rule fired despite a near-zero model prediction

    detector2 = RepricingDetector()
    state2 = StateManager(state_file=str(tmp_path / "state2.json"))
    high_confidence_model = FakeModel(probability=0.99)
    qsg2 = QuantSignalGenerator(detector2, state2, fetcher, dataset=FakeDataset(), model=high_confidence_model)
    no_signal = qsg2.process_market(make_market())  # no price-drop history seeded
    assert no_signal is None  # repricing rule didn't fire despite a near-certain model prediction


def test_process_market_logs_signal_with_features_and_shadow_model_probability(tmp_path):
    detector = RepricingDetector()
    state = StateManager(state_file=str(tmp_path / "state.json"))
    fetcher = FakeFetcher(order_book_top={"best_bid_price": 0.49, "best_ask_price": 0.51,
                                           "total_bid_depth": 100.0, "total_ask_depth": 50.0})
    dataset = FakeDataset()
    model = FakeModel(probability=0.62)
    qsg = QuantSignalGenerator(detector, state, fetcher, dataset=dataset, model=model)
    ...
    assert logged["model_probability"] == 0.62
    assert logged["features"]["yes_price"] == 0.5
    assert logged["features"]["order_book_imbalance"] is not None
    assert "m1" in qsg._pending
    # the signal itself reflects the repricing rule's own confidence, not the model's
    assert signal.confidence != model.probability


def test_process_market_logs_signal_with_none_model_probability_when_no_model(tmp_path):
    detector = RepricingDetector()
    state = StateManager(state_file=str(tmp_path / "state.json"))
    fetcher = FakeFetcher(order_book_top={"best_bid_price": 0.49, "best_ask_price": 0.51,
                                           "total_bid_depth": 100.0, "total_ask_depth": 50.0})
    dataset = FakeDataset()
    qsg = QuantSignalGenerator(detector, state, fetcher, dataset=dataset, model=None)
    # model=None in the constructor means "not provided, auto-load from disk",
    # not "explicitly disabled" -- force the no-model case directly so this
    # test doesn't depend on whether data/quant_model.pkl happens to exist.
    qsg.model = None

    fire_repricing_signal(detector)
    qsg.process_market(make_market(yes_price=0.5, no_price=0.5))

    assert dataset.signals[0]["model_probability"] is None
...
def test_process_market_does_not_log_when_no_signal(tmp_path):
    ...
    fetcher = FakeFetcher()
    dataset = FakeDataset()
    qsg = QuantSignalGenerator(detector, state, fetcher, dataset=dataset, model=FakeModel(0.99))
...
def test_resolve_pending_logs_win_outcome_and_clears_pending(tmp_path):
    ...
    qsg._pending["m1"] = {"asset": "BTC", "features": {"yes_price": 0.4}, "entry_price": 0.4,
                           "direction": "YES", "model_probability": 0.65}
    ...
    assert dataset.resolutions[0]["model_probability"] == 0.65
